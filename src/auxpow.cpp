...
bool
CDefaultAuxPow::check (const uint256& hashAuxBlock, int nChainId,
...
int
CDefaultAuxPow::getExpectedIndex (uint32_t nNonce, int nChainId, unsigned h)
{
...
uint256
CDefaultAuxPow::CheckMerkleBranch (uint256 hash,
...
void
CDefaultAuxPow::initAuxPow (CBlockHeader& header)
{
  ...
  assert (header.auxpowdefault->vChainMerkleBranch.empty ());
...
bool
CEquihashAuxPow::check (const uint256& hashAuxBlock, int nChainId,
                const Consensus::Params& params) const
{
    if (nIndex != 0)
        return error("AuxPow is not a generate");
...
int
CEquihashAuxPow::getExpectedIndex (uint32_t nNonce, int nChainId, unsigned h)
{
  // Choose a pseudo-random slot in the chain merkle tree
  // but have it be fixed for a size/nonce/chain combination.
  //
...
uint256
CEquihashAuxPow::CheckMerkleBranch (uint256 hash,
                            const std::vector<uint256>& vMerkleBranch,
                            int nIndex)
{
  if (nIndex == -1)
...
CEquihashAuxPow::initAuxPow (CBlockHeader& header)
{
  ...
  assert (header.auxpowequihash->vChainMerkleBranch.empty ());
