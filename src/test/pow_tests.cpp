...
BOOST_AUTO_TEST_CASE(GetBlockProofEquivalentTime_test)
{
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        blocks[i].nHeight = i;
        blocks[i].nTime = 1269211443 + i * chainParams->GetConsensus().nPowTargetSpacingV2;
        blocks[i].nBits = 0x207fffff; /* target 0x7fffff000... */
        blocks[i].nChainWork = i ? blocks[i - 1].nChainWork + GetBlockProof(blocks[i - 1], chainParams) : arith_uint256(0);
    }
