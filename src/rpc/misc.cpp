...
UniValue listattackersaddresses(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "listattackersaddresses\n"
            "\nList all blocked addresses, that has been marked as unspendable.\n"
            "\nArguments:\n"
            "(none)\n"
            "\nResult:\n"
            "{\n"
    ...
    ret.pushKV("addresses", arr);
    return ret;
}

UniValue convertoldscriptaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "convertoldscriptaddress \"address\"\n"
            "\nConvert an old Script address, to the new script address format.\n"
            "\nArguments:\n"
            "1. \"address\"                          (string, required) The old globaltoken script address to convert\n"
            "\nResult:\n"
            "{\n"
            "  \"succeeded\" : true|false,           (boolean) If the conversion succeeded.\n"
            "  \"oldaddress\" : \"oldaddress\",        (string) The old globaltoken script address\n"
            "  \"newaddress\" : \"newaddress\",        (string) The new globaltoken script address\n"
            "  \"scriptPubKey\" : \"scriptPubKey\",    (string) The hex encoded scriptPubKey generated by the address\n"
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("convertoldscriptaddress", "\"3dCvo5ZYRVHamNmSrxanyZdiR86krcH22j\"")
            + HelpExampleRpc("convertoldscriptaddress", "\"3dCvo5ZYRVHamNmSrxanyZdiR86krcH22j\"")
        );

    CTxDestination dest = DecodeDestination(request.params[0].get_str());
    bool isValid = IsValidDestination(dest);
    bool isOldScriptAddress = IsDestinationStringOldScriptFormat(request.params[0].get_str());
    bool fsucceeded = isValid && isOldScriptAddress;

    UniValue ret(UniValue::VOBJ);
    ret.pushKV("succeeded", fsucceeded);
    if(!isOldScriptAddress)
    {
        ret.pushKV("error", "Given address is not an old script address!");
    }
    if (fsucceeded)
    {
        std::string inputOldAddress = EncodeOldScriptDestination(dest);
        ret.pushKV("oldaddress", inputOldAddress);
        std::string newAddress = EncodeDestination(dest);
        ret.pushKV("newaddress", newAddress);
...
UniValue helpnewscriptaddress(const JSONRPCRequest& jsonRequest)
{
    if (jsonRequest.fHelp || jsonRequest.params.size() > 0)
        throw std::runtime_error(
            ...
            "\n(none)\n"
            "\nResult:\n"
            "\"text\"     (string) The information text about the new script address format\n"
        );

    return GetOldScriptAddressWarning();
}
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "transition to using getaddressinfo to access this information before upgrading to v2.9(+). The following deprecated\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("validateaddress", "\"GdCvo5ZYRVHamNmSrxanyZdiR86krcH22j\"")
            + HelpExampleRpc("validateaddress", "\"GdCvo5ZYRVHamNmSrxanyZdiR86krcH22j\"")
        );
    ...
    UniValue ret(UniValue::VOBJ);
    ret.pushKV("isvalid", isValid && !isOldScriptAddress);
    if(isOldScriptAddress)
    {
        ret.pushKV("error", "Address is valid, but it is in old script address format. Convert it into the new script address format, in order to use it! For more run the RPC-command 'helpnewscriptaddress'");
    }
    if (isValid && !isOldScriptAddress)
    {
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    for (unsigned int i = 0; i < keys.size(); ++i) {
        if (IsHex(keys[i].get_str()) && (keys[i].get_str().length() == 66 || keys[i].get_str().length() == 130)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("Invalid public key: %s\nNote that from v2.1, createmultisig no longer accepts addresses."
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "util",               "helpnewscriptaddress",   &helpnewscriptaddress,   {} },
    { "util",               "convertoldscriptaddress",&convertoldscriptaddress,{"address"} },
    { "util",               "verifymessage",          &verifymessage,          {"address","signature","message"} },
    { "util",               "signmessagewithprivkey", &signmessagewithprivkey, {"privkey","message"} },
    { "util",               "listattackersaddresses", &listattackersaddresses, {} },
