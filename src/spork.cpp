...
std::map<int, int64_t> mapSporkDefaults = {
    ...
    {SPORK_7_RECONSIDER_BLOCKS,              0},             // 0 BLOCKS
};
...
int CSporkManager::GetSporkIDByName(const std::string& strName)
{
    ...
    if (strName == "SPORK_4_NEW_SIGS")                          return SPORK_4_NEW_SIGS;
    if (strName == "SPORK_5_MASTERNODE_PAYMENT_ENFORCEMENT")    return SPORK_5_MASTERNODE_PAYMENT_ENFORCEMENT;
    if (strName == "SPORK_6_MASTERNODE_PAY_UPDATED_NODES")      return SPORK_6_MASTERNODE_PAY_UPDATED_NODES;
...
std::string CSporkManager::GetSporkNameByID(int nSporkID)
{
    switch (nSporkID) {
        ...
        case SPORK_4_NEW_SIGS:                          return "SPORK_4_NEW_SIGS";
        case SPORK_5_MASTERNODE_PAYMENT_ENFORCEMENT:    return "SPORK_5_MASTERNODE_PAYMENT_ENFORCEMENT";
        case SPORK_6_MASTERNODE_PAY_UPDATED_NODES:      return "SPORK_6_MASTERNODE_PAY_UPDATED_NODES";
...
bool CSporkMessage::CheckSignature(const CKeyID& pubKeyId) const
{
    ...
    if (sporkManager.IsSporkActive(SPORK_4_NEW_SIGS)) {
        ...
        if (!CHashSigner::VerifyHash(hash, pubKeyId, vchSig, strError)) {
            // Note: unlike for many other messages when SPORK_4_NEW_SIGS is ON sporks with sigs in old format
        ...
        if (!CMessageSigner::VerifyMessage(pubKeyId, vchSig, strMessage, strError)){
            // Note: unlike for other messages we have to check for new format even with SPORK_4_NEW_SIGS
            // inactive because SPORK_4_NEW_SIGS default is OFF and it is not the first spork to sync
