...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, const CPubKey& pubkey, int& nHeightRet)
{
    ...
    if(coin.out.nValue != Params().GetConsensus().nMasternodeColleteralPaymentAmount * COIN) {
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if (err == COLLATERAL_INVALID_AMOUNT) {
        LogPrint(BCLog::MASTERNODE, "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %d GLT, masternode=%s\n", Params().GetConsensus().nMasternodeColleteralPaymentAmount, outpoint.ToStringShort());
        nDos = 33;
        return false;
    }
